...
def audio_tagging(args):
    ...
    output_filename = "my_rating_ana_results.csv"
    ...
    #print(prev)
    found_names = [os.path.join(dp, f) for dp, dn, fn in os.walk(audio_path) for f in fn]#os.listdir(audio_path)
    print(f"orig len: {len(found_names)}")
    found_names = [f for f in found_names if (((".mp3" in f) or (".m4a" in f)) or ".flac" in f) and f not in prev]
    print(f"now have {len(found_names)} songs")
    print(found_names[0:10])
    #print(found_names[0:10])
    count = 0
    ...
    for song_filename in found_names:
        ...
        try:
            ...
            if len(rawaudio) < 230400000:
                ...
                with torch.no_grad(), autocast(device_type=device.type) if args.intel else nullcontext():
                    spec = mel(waveform)
